...
import { AlertTriangle, Bell, BookOpen, CalendarX2, Clock, Loader2, MapPin, RefreshCw } from "lucide-react";
...
async function fetchStudentTimetable({ semester, branch, section }) {
  const response = await fetch(`http://localhost:8080/api/timetable/student`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ "semester": semester, "branch": branch, "sectionNo": section }),
  });
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-lg border-2 border-dashed border-slate-300 bg-white py-20 text-center">
        ...
        <p className="max-w-xs text-sm text-slate-400">Nothing came back for this semester/branch/section — try refreshing.</p>
      </div>
    );
  }
    ...
    <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      >
        ...
        {entries.map((e) => {
          ...
          const room = e.classroomName ?? e.classroomNo;
          ...
          return (
            <div
              ...
              title={`${e.courseName} · ${e.teacherName} · ${room}`}
            >
              ...
              <div className={`flex flex-1 flex-col justify-center gap-0.5 px-1.5 py-0.5 ${style.text}`}>
                ...
                <span className="truncate text-[10px] leading-tight opacity-90">{e.teacherName}</span>
...
export default function TimetablePage() {
  ...
  const {
    studentName: studentNameFromState,
  ...
  const [session, setSession] = useState(() => {
    if (initialDetails) {
      return { details: initialDetails, studentName: studentNameFromState, studentId: studentIdFromState, token: tokenFromState };
    }
    return readCachedSession();
  });

  const studentDetails = session?.details;
  const studentName = studentDetails?.data.studentName;
  ...
  useEffect(() => {
    if (initialDetails) {
      const next = { details: initialDetails, studentName: studentNameFromState, studentId: studentIdFromState, token: tokenFromState };
      setSession(next);
      try {
  ...
  }, [initialDetails, studentNameFromState, studentIdFromState, tokenFromState]);

  const [timetableResponse, setTimetableResponse] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const studentInfo = studentDetails?.data;

  const loadTimetable = useCallback(async () => {
    // Guard: don't call the API (and don't crash) if we don't actually
    // have the semester/branch/section yet.
    if (!studentInfo?.semester || !studentInfo?.branch || !studentInfo?.section) {
      setTimetableResponse([]);
      setError("Missing student semester, branch, or section. Please log in again.");
      return;
    }
  ...
  }, [studentInfo?.semester, studentInfo?.branch, studentInfo?.section]);
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl">Student Timetable</h1>
              <p className="text-sm text-slate-600">
                {studentName ? `Welcome, ${studentName}` : "Fetches and prints the timetable entries returned by the backend."}
                {studentId ? ` (${studentId})` : ""}
              </p>
            </div>
          </div>
        ...
        <p className="mt-3 flex items-center gap-1.5 text-xs text-slate-400">
          <Clock size={12} /> Grid spans {GRID_DAY_START}–{GRID_DAY_END}, Monday through Saturday.
        </p>
